...
var domEvents = function () {
    function domEvents() {
        ...
        this.playerCardBlock = (0, _helpers.$sel)('.player__card');
        this.playerCardFront = (0, _helpers.$sel)('.player__card--back');
        this.playerWarCardBlock = (0, _helpers.$sel)('.player__war-card');
        this.playerResultBlock = (0, _helpers.$sel)('.result-block__player');
        this.views = (0, _helpers.$sel)('section');
    }

    _createClass(domEvents, [{
        key: 'showView',
        value: function showView(view) {
            [].map.call(this.views, function (el) {
                return el.classList.add('hidden');
            });
            (0, _helpers.$sel)('#' + view).classList.remove('hidden');
        }
    }, {
        key: 'createCard',
        value: function createCard(src) {
        ...
        value: function showCards(cards) {
            ...
            if (cards.length === 4) {
                for (var i = 0; i < this.playerWarCardBlock.length; i++) {
            ...
            for (var _i = 0; _i < this.playerCardFront.length; _i++) {
                this.playerCardFront[_i].appendChild(this.createCard(cards[_i].image));
            }
            // dodanie animacji
            setTimeout(function () {
        ...
        value: function removeCards() {
            [].map.call(this.playerCardBlock, function (el) {
                return el.classList.remove('flipped');
            });
            [].map.call(this.playerCardFront, function (el) {
                return el.innerHTML = '';
            });
            [].map.call(this.playerWarCardBlock, function (el) {
                return el.innerHTML = '';
            });
        }
        ...
        value: function updateResult() {
            var diff = (window.storage.players[0].cards.length - window.storage.players[1].cards.length) / 2;

            // aktualizacja wyniku (26 - połowa talii)
            [].map.call(this.playerResultBlock, function (el) {
                el.innerHTML = 26 + diff;
                diff = -diff;
            });

            if (window.DEBUG) {
                console.log('Wynik: ', (0, _helpers.$sel)('#result-player-1').innerHTML + ' vs ' + (0, _helpers.$sel)('#result-player-2').innerHTML);
            }
            (0, _helpers.$sel)('#result-rounds').innerHTML = window.storage.rounds;
        }
    }, {
        key: 'hideElement',
        value: function hideElement(selector) {
            [].map.call((0, _helpers.$sel)(selector), function (el) {
                return el.classList.add('hidden');
            });
        }
    }, {
...
var _domEvents = require('./domEvents');
...
var domEvents = new _domEvents2.default();
...
var game = function () {
    function game() {
        ...
        this.nicknameInput = (0, _helpers.$sel)('#nickname');
        this.nicknameFields = (0, _helpers.$sel)('.your-nickname');
    }
    ...
    _createClass(game, [{
        ...
        value: function storageInit() {
            window.storage = {
                rounds: 0,
                players: [{ name: 'Computer', cards: [] }, { name: 'Player', cards: [] }]
            };
        }
    }, {
        key: 'bindEvents',
        value: function bindEvents() {
            ...
            var that = this;
            (0, _helpers.$sel)('#start-game').addEventListener('click', function (e) {
                return _this.startNewGame(e);
            });
            (0, _helpers.$sel)('#end-game').addEventListener('click', function (e) {
                return _this.endGame(e);
            });
            (0, _helpers.$sel)('#restart-game').addEventListener('click', function (e) {
                return _this.restartGame(e);
            });

            (0, _helpers.$sel)('.player__pile')[1].addEventListener('click', function () {
        ...
        value: function startNewGame(e) {
            ...
            $http.get('new/shuffle/?deck_count=1').then(function (result) {
                ...
                [].forEach.call(that.nicknameFields, function (el) {
                    return el.innerHTML = that.getNickname();
                });
            });
        }
    }, {
        ...
        value: function endGame(e) {
            ...
            [].forEach.call((0, _helpers.$sel)('.outro__summary'), function (el) {
                return el.classList.add('hidden');
            });
            // pokaż odpowiednie info w zależności od wyniku
            if (window.storage.players[0].cards > window.storage.players[1].cards) {
                (0, _helpers.$sel)('#outro-failed').classList.remove('hidden');
            } else if (window.storage.players[0].cards < window.storage.players[1].cards) {
                (0, _helpers.$sel)('#outro-success').classList.remove('hidden');
            } else {
                (0, _helpers.$sel)('#outro-draw').classList.remove('hidden');
            }
        }
    }, {
        key: 'getCards',
        value: function getCards(number, deckId) {
            var that = this;
        ...
        key: 'getCardsFromPile',
        value: function getCardsFromPile(number) {
            var player = 0,
                cards = [];
        ...
        key: 'beginBattle',
        value: function beginBattle(cards) {
            if (window.DEBUG) {
            ...
            if (cards.slice(-2)[0].value === cards.slice(-1)[0].value) {
                ...
                if (this.prevCards) {
                    cards.unshift.apply(cards, _toConsumableArray(this.prevCards));
        ...
        key: 'getBattleWinner',
        value: function getBattleWinner(cards) {
            var that = this;
        ...
        key: 'getFigureValue',
        value: function getFigureValue(figure) {
            return isNaN(+figure) ? this.figures[figure] : parseInt(figure);
        }
    }, {
        key: 'updateUserCards',
        value: function updateUserCards(cards) {
            var _window$storage$playe;

            // wybranie zwycięzcy na podstawie ostatniej pary kart
            var index = this.getBattleWinner(cards.slice(-2));

            // dodanie kart do kolekcji zwycięzcy
            (_window$storage$playe = window.storage.players[index].cards).push.apply(_window$storage$playe, _toConsumableArray(cards));
            domEvents.updateResult();
...
},{"./domEvents":3,"./helpers":5,"./http":6}],5:[function(require,module,exports){
'use strict';
